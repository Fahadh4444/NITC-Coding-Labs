...
int func (int i  , struct node *node , int k ){
   int  sum = 0 ;
   sum +=  100  ; 
...
void Cut(struct heap*h , struct node* node, struct node* temp){
    if (node == node->right)
        temp->child = NULL;   
        int k = func(0, temp , 20); 
...
struct node * Decrease_key(struct heap *h ,struct node* node, int val){
    node->key = val;
    struct node* temp = node->p;
...
int Delete (struct heap *h , struct node *node ){
    if (h->min == NULL )
        return -1 ;
    struct node *temp ;
    int res = node->key ;
    temp =  Decrease_key(h,node,99999);
    temp = Extract_min(h);
    return res ;
}
